...
// shared resources
unique_ptr<Mesh> Crown1::mesh;
unique_ptr<Shader> Crown1::shader;
unique_ptr<Texture> Crown1::texture;
std::vector<tinyobj::material_t> Crown1::mat;
std::map<std::string, int> Crown1::mat_map;
...
bool Crown1::update(Scene &scene, float dt) {
  for (auto &obj : scene.objects) {
    auto player = dynamic_cast<Player1 *>(obj.get());
    if(player){
      rotation.y += dt;
      modelMatrix =
...
void Crown1::player(bool x){
  player_number = x;
}

bool Crown1::get_player(){
  return player_number;
}

void Crown1::render(Scene &scene) {
